 ...
 * Inspiration: https://github.com/aspt/mp4
 */
...
typedef struct{
    ring_buffer_vi_t * ring;
    uint32_t baseMediaDecodeTime; 
    uint32_t sequenceNumber;
    uint8_t SPSNALU[128];
    uint32_t SPSNALUNumBytes;
    uint8_t PPSNALU[128];
    uint32_t PPSNALUNumBytes;
    uint32_t ring_trun_index;
    uint32_t ring_mdat_index;
} POSMp4VideoTrack;
...
typedef struct{
    ...
    ring_buffer_vi_t * ring;
    uint32_t baseMediaDecodeTime;
    uint8_t DSIBYTES[128];
    uint32_t DSINumBytes;
    uint32_t ring_trun_index;
    uint32_t ring_mdat_index;
    bool mute;
} POSMp4AudioTrack;
...
enum
{
    ...
    BOX_tfdt    = FOUR_CHAR_INT( 't', 'f', 'd', 't' ),//TrackFragmentDecodeTimeAtomType
